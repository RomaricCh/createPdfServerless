...
```bash
CreatePdfServerless$ make build
CreatePdfServerless$ make deploy.guided
```
...
```bash
CreatePdfServerless$ make invoke
```
...
```bash
CreatePdfServerless$ make run
CreatePdfServerless$ curl http://localhost:3000/hello
```
...
```bash
CreatePdfServerless$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
CreatePdfServerless$ make dev
CreatePdfServerless$ make test
```
...
```bash
CreatePdfServerless$ aws cloudformation delete-stack --stack-name CreatePdfServerless
```
...
```bash
CreatePdfServerless$ pipenv lock -r > hello_world/requirements.txt
```
